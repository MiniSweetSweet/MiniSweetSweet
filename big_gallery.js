...
/***** PHOTO DESCRIPTION  *****/  
	  ...
	  var slideshow = cfs.configuration("http://www.artecasauae.com/gallery/js/auto.play");
		if($('#desc_'+ $(this).attr('class')).html()) {
			$('#desc_'+ $(this).attr('class')).modal({
				...
				onShow: function() {
						if(slideshow==true)
						{
								cfs.configuration("http://www.artecasauae.com/gallery/js/auto.play", false);
				...
				onClose: function() {
						...
						if(slideshow==true)
						{
								cfs.configuration("http://www.artecasauae.com/gallery/js/auto.play", true);
								$gallery.trigger("play", true);
						}
						$.modal.close();
						$slider_wrapper.css({'height':this_wrapper_h});
...
/***** TWEETIE  *****/  
	...
	if($('#tweets').length) {
            ...
            count: 5,
			apiPath : 'http://www.artecasauae.com/gallery/js/js/tweetie/api/tweet.php'
        } , function () {
            api.reinitialise();
			$('.page-footer').children(".shadow-reverse").css("top", $('.page_block').innerHeight() -$('.shadow-reverse').innerHeight()+ 'px'); 
        });
